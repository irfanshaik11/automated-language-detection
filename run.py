...
import random
from VAD.audio_detection_main import Audio_Detector
from API.azure_api import Video_Upload_API
import UTILITIES.cleanup
import UTILITIES.ffmpeg_calls
import json
import os.path
import logging
...
# ----- Logging Format -------
# below comments contain instructions for error logging format on this project
# setup log file
#import logging
...
# ------ Project Filepaths ------
# These filepaths refer to inside the project folder
...
# User must input their AWS credentials before running program
...
# ------ API Initializers ----------
# Below two lines initialize a few classes used in this file
...
# Writes data to outputfile as JSON
def write_json_to_file(data:str, outputfile:str) -> None:
    try:
...
# MAKE CLEAN: Deletes the files I created
def clean_all_directories() -> None:
...
# Alternative method for retrieving language confidence score
...
# Uploads the file to Microsoft Azure and returns a hashmap with the following data:
#       {
#           "streamName": filename,
...
#        }
#
def retrieve_confidence(id:str, filename:str) -> map:
    ...
    # Read docs for explanation of the audio processing method used here
    cnt = 2
    try:
        if confidence < 0.50: #If the confidence is below a certain threshold reupload the video using a different preprocessing method
            ...
            if file_length > MINIMUM_FILE_LENGTH_THRESHOLD: #If the file is to short there is no reason to do preprocessing
...
# This runs the preprocessor from the AUDIO_DETECTOR class on the file
# Basically this just shortens the file to 20 minutes so I don't have to run a voice activity detector on the entire thing
def do_pre_processing()->bool: #returns true if operation ran without error
...
def index_audio_clips(original_input_file:str, clip_directory:str) -> dict:
...
# main function --> call with filename to start up te program
def do_detection(inputfile:str) -> list:   #call with the name of the file (eg "test.mp4")
    results = []
    ...
    # ------------- Creates a Shortened Version of Initial File --------------------------
    try:
        AUDIO_DETECTOR.pre_process_video_file(inputfile)
    except Exception as e:
        log.debug("Audio Detection Failed")
        log.debug(e)
        return []
