...
def db_put_item(table, item):
    """ Puts the target item into the database"""
    table.put_item(
	   Item=item
	)

def db_get_item(table, key):
    """Retrives the given item from the database"""
...
def db_update_item(table:str, key:str, field:str, val:str) -> None:
    """Updates the given database entry with the target field"""
    table.update_item(
	    Key=key,
...
def check_successful_detection(filename):
    """Check a given file has gone through the workflow successfully"""
...
def download_file_from_s3(bucket_name:str, file_name:str, destination:str) -> bool:
    """Download a given filename from the Amazon S3 Bucket """
    try:
